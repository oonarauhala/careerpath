...
struct CareerResponse:Decodable {
    ...
    func convertToCareer() -> Career {
        return Career(careerName: self.name, description: self.description, medianSalary: Double(self.salary), education: convertToDegree(intValue: self.degree), personalityType: convertToPersonalityType(intValue: self.personality))
    }
